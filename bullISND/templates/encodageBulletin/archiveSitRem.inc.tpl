			...
			<div id="collapseRem{$matricule}" class="accordion-body collapse" style="height: 0px; ">
				<div class="accordion-inner">
					...
					{assign var="periode" value=$smarty.section.annee.index}
						<li>{$periode} <i class="fa fa-long-arrow-right"></i> {$listeCommentaires.$matricule.$periode|default:Null}
					{/section}
					</ul>
				</div>  <!-- accordion-inner -->
			</div>  <!-- accordion-body -->
			...
			<div id="collapseSit{$matricule}" class="accordion-body collapse" style="height: 0px; ">
				<div class="accordion-inner">
					...
					{section name=annee start=1 loop=$nbBulletins+1}
						...
						<li>{$periode} <i class="fa fa-long-arrow-right"></i> {if $situation.sit}<strong>{$situation.sit}/{$situation.max}</strong> soit {$situation.pourcent}%{/if}</li>
					{/section}
					</ul>
				</div>  <!-- accordion-inner -->
			</div>  <!-- accordion-body -->
