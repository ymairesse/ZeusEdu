				...
				<form name="formPonderations" method="POST" action="index.php" id="formPonderations" class="form-vertical" role="form">
					...
					{assign var=n value=$smarty.section.periodes.index}
						<tr>
							...
							<td>
								<input type="text" class="poids form-control" value="{$ponderation.$n.form}"
									   {if ($bulletin > $n)} disabled {/if}
									   name="formatif_{$n}" maxlength="3"
							</td>
							<td>
								<input type="text" class="poids form-control" value="{$ponderation.$n.cert}"
									{if ($bulletin > $n)} disabled {/if} 
									name="certif_{$n}" maxlength="3"
							</td>
						</tr>
					{/section}
					</table>
