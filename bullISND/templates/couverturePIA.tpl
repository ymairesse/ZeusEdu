...
<div id="blocNom" class="shadow">
	<img src="../photos/{$eleve.detailsEleve.photo}.jpg" style="height:150px; float:left;" alt="{$eleve.photo}">
	<br>
	<div id="nomPrenom">
		<h3>Ce document appartient à </h3>
		Nom: <strong>{$eleve.detailsEleve.nom}</strong> <br>
		Prénom: <strong>{$eleve.detailsEleve.prenom}</strong><br>
		Classe: <strong>{$eleve.detailsEleve.groupe}</strong>
	</div>
</div>
...
<p class="coursProf">Adresse: <strong>{$eleve.detailsEleve.adresseEleve}</strong><br>
			...
			{if in_array($coursGrp, array_keys($eleve.resultats.$periode))}
				<td class="cote" title="{$coursGrp}">
					{if isset($eleve.resultats.$periode.$coursGrp.sitDelibe) && ($eleve.resultats.$periode.$coursGrp.sitDelibe != '')}
						...
						<strong>{$eleve.resultats.$periode.$coursGrp.sitDelibe}</strong><br>
					{/if}
					
					{if isset($eleve.resultats.$periode.$coursGrp.pourcent) && ($eleve.resultats.$periode.$coursGrp.pourcent != '') }
					{$eleve.resultats.$periode.$coursGrp.situation}/{$eleve.resultats.$periode.$coursGrp.maxSituation}<br>
					<span class="micro">={$eleve.resultats.$periode.$coursGrp.pourcent}</span>
					{/if}
				 </td>
				{else}
				<td>&nbsp;</td>
