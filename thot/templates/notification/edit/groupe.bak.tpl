<!-- Notifications à un groupe -->

	{assign var=liste value=$listeNotifications.groupe}
	<table class="table table-condensed tableEdit">
		...
		<tr id="tr_{$nId}" data-id="{$nId}" data-type="classe">
			<td style="width:1em">
				<input type="checkbox" class="checkDelete" id="check{$nId}" data-id="{$nId}" data-type="groupe">
			</td>
			<td style="width:1em">
				<button type="button" class="btn btn-default btnEdit btn-xs pop" data-id="{$nId}" data-type="groupe" data-content="Modifier cette annonce">
			...
			<td style="width:1em">
				<button type="button" class="btn btn-danger btn-delete btn-xs" data-id="{$nId}" data-type="groupe">
			...
			<td>
				{if $uneNote.accuse == 1}
				<meter style="float:left; margin-right:0.5em" title="Cliquer pour voir le détail" data-container="body" class="showAccuse" value="{$listeAccuses.$nId|count|default:0}" min="0" max="{$listeAttendus.groupe.$nId}" data-id="{$nId}"></meter>
				<span class="discret">{$listeAccuses.$nId|count|default:0}/{$listeAttendus.groupe.$nId}</span>
				{else}N/A
				{/if}
			</td>
		...
		<tr>
			<th colspan="10">
				...
				<button class="btn btn-warning delModal" type="button" data-type="groupe">
