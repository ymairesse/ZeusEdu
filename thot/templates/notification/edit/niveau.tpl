...
<div class="tab-pane hidden-print" id="tabs-4">
	...
	<table class="table table-condensed tableEdit">
		...
		<tr id="tr_{$nId}" data-id="{$nId}" data-type="niveau">
			...
			<td style="width:20%;">
				<span class="destinataire">Toutes les {$uneNote.destinataire}e</span>
			</td>
			<td>
				{if $uneNote.accuse == 1}
				<meter style="float:left; margin-right:0.5em" title="Cliquer pour voir le détail" data-container="body" class="showAccuse" value="{$listeAccuses.$nId|count|default:0}" min="0" max="{$listeAttendus.niveau.$nId}" data-id="{$nId}"></meter>
				<span class="discret">{$listeAccuses.$nId|count|default:0}/{$listeAttendus.niveau.$nId}</span>
				{else}N/A
				{/if}
			</td>
		...
		<tr>
			<th colspan="10">
				...
				<button class="btn btn-warning delModal" type="button" data-type="niveau">
