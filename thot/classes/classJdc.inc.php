...
class Jdc {
    ...
    * @param integer $from : date de début = timestamp Unix en millisecondes
    * @param integer $to : date de fin = timestamp Unix en millisecondes
    ...
    * @return integer : le nombre de suppression (0 -si échec- ou 1)
    */
    ...
    */
    public function saveJdc($post,$acronyme){
        ...
        $allDay = isset($post['journee'])?true:false;
        ...
        if ($allDay == false) {
            ...
            if (($duree <= 20) || ($duree > 400))
                $duree = 20;  // 20 minutes en cas d'erreur et pour la lisibilité
