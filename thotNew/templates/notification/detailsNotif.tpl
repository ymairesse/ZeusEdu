...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		// 				$.post('inc/notification/refreshNotifications.inc.php', {
		// 					type: type
		// 				}, function(resultat){
		...
		$('#ficheEleve').on('change', '#type', function(){
			...
			switch (type) {
				case 'ecole':
					...
					$.post('inc/notification/noListeEleves.inc.php', {
						destinataire: 'École'
					}, function(resultat){
		...
		$('#ficheEleve').on('change', '#niveau4classe', function(){
			var niveau = $(this).val();
			$.post('inc/notification/selectClasseNiveau.inc.php', {
				niveau: niveau
			}, function(resultat){
		...
		$('#ficheEleve').on('change', '#niveau4matiere', function(){
			var niveau = $(this).val();
			$.post('inc/notification/selectMatiereNiveau.inc.php', {
				niveau: niveau
			}, function(resultat){
		...
		$('#ficheEleve').on('change', '#classe', function(){
			var classe = $(this).val();
			$.post('inc/notification/listeElevesClasse.inc.php', {
				classe: classe
			}, function(resultat){
				$('#choixEleves').html(resultat).removeClass('hidden');
				$('.cb').prop('checked', false);
		...
		$('#ficheEleve').on('change', '#selectCoursGrp', function(){
			...
			$.post('inc/notification/listeElevesCoursGrp.inc.php', {
				coursGrp: coursGrp
			}, function(resultat){
				$('#choixEleves').html(resultat).removeClass('hidden');
				$('.cb').prop('checked', false);
		...
		$('#ficheEleve').on('click', '.btnEdit', function(){
			...
			$.post('inc/notification/editNotification.inc.php', {
				notifId: notifId
			}, function(resultat){
		...
		$("#ficheEleve").on('click', '.btn-delete', function() {
			...
			$.post('inc/notification/listePjFiles.inc.php', {
				notifId: notifId
				},
				function(resultat) {
		...
		$('#corpsPage').on('click', '.delPJ', function(){
			...
			var bouton = $(this);
			$.post('inc/notification/unlinkShare4Notif.inc.php', {
				shareId: shareId,
				notifId: notifId},
			function(){
		...
		// 	$.post('inc/notification/delId.inc.php', {
		// 			notifId: notifId,
		// 			form: form
		// 		},
		// 		function(resultat) {
		...
		$("#ModalDelBulkBtn").click(function() {
			...
			$.post('inc/notification/delBulk.inc.php', {
					listeId: listeId
				},
				function(resultat) {
					$.each(cbId, function(i) {
