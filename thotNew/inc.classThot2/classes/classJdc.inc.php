...
// Application::afficher(array($start, $end, $destinataire, $type));
        if ($acronyme != Null)
            $requete->bindParam(':acronyme', $acronyme, PDO::PARAM_STR, 7);
    ...
    public function getRemediation($id) {
        ...
        if ($resultat){
            ...
            $travail['categorie'] = 'Remédiation';
            $travail['duree'] = substr($travail['duree'], 0, 5);
        }
    ...
    public function getTravailFromGhost($idGhost, $acronyme){
        ...
        if ($resultat){
            $requete->setFetchMode(PDO::FETCH_ASSOC);
            $travail = $requete->fetch();
            $travail['cible'] = $travail['destinataire'];
            $travail['profs'] = ($travail['sexe'] == 'F') ? 'Mme ' : 'M. ';
            $travail['profs'] .= mb_substr($travail['prenom'], 0, 1, 'UTF-8').'. '.$travail['nom'];
        }
    ...
    public function saveJdc($post, $acronyme)
    {
        ...
        $cible = isset($post['cible']) ? $post['cible'] : Null;
        ...
        $requete = $connexion->prepare($sql);
// echo $sql;
        $requete->bindParam(':destinataire', $cible, PDO::PARAM_STR, 20);
        $requete->bindParam(':type', $type, PDO::PARAM_STR, 10);
