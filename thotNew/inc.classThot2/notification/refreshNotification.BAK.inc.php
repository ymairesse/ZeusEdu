...
if ($form['id'] != '') {
    ...
    switch ($form['type']) {
        case 'classes':
            $listeEleves = isset($form['TOUS']) ? $Ecole->listeElevesClasse($destinataire) : array_flip($form['membres']);
            break;
        case 'coursGrp':
            $listeEleves = isset($form['TOUS']) ? $Ecole->listeElevesCours($destinataire) : array_flip($form['membres']);
            break;
        case 'groupe':
...
if (isset($form['mail']) && $form['mail'] == 1) {
    ...
    $texte[] = sprintf('%d mail(sà envoyé(s)', count($listeEnvois));
}
