...
Application::afficher($listeAlreadyShared);
...
Application::afficher($listeAllShared);
    // liaison des PJ existantes avec redistribution aux destinataires
    // la "$cible" peut être un groupe entier (Exemple: classe 3Z) ou un matricule d'élève (Exemple: 1234)
    Application::afficher($listeAllShared, true);
    foreach ($listeNotifId AS $cible => $notifId) {
        foreach ($listeAllShared AS $fileId => $listeDestinatairesShares) {
            foreach($listeDestinatairesShares AS $destinataire => $dataShare){
				$shareId = $dataShare['shareId'];
				Application::afficher(array($cible, $notifId, $destinataire, $shareId));
                // on attribue le fichier $shareId à un élève (par son matricule) ou à tout le groupe ('all')
                if (($destinataire == $cible) || ($destinataire == 'all'))
                    $Thot->linkNotifPJ($notifId, $shareId);
                }
            }
        }
