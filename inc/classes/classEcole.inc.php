...
class ecole {
    ...
    public static function titusDeGroupe($groupe) {
        if ($groupe	== Null) die("missing group");
        $connexion = Application::connectPDO (SERVEUR, BASE, NOM, MDP);
        ...
        if ($compact) {
            foreach ($listeGroupes as $nomGroupe=>$classes) {
                if (count($classes) > 1)
                    ...
                    else unset($listeGroupes[$nomGroupe]);
            }
        }
	return $listeGroupes;
    }
	...
	*/
	 ...
	 */
	 public function listeCoursListeClasses($listeClasses) {
		...
		if (is_array($listeClasses))
			...
			else $listeClassesString = $listeClasses;
		$sql = "SELECT DISTINCT ".PFX."elevesCours.coursGrp, ";
		$sql .= "SUBSTR(".PFX."elevesCours.coursGrp, 1,LOCATE('-',".PFX."elevesCours.coursGrp)-1) AS cours, ";
		$sql .= PFX."profsCours.acronyme, ".PFX."profs.nom, ".PFX."profs.prenom, nbheures, libelle ";
		$sql .= "FROM ".PFX."elevesCours ";
	...
	public static function listeCoursListeSections ($listeSections) {
		...
		if (is_array($listeSections))
			...
			else $listeSectionsString = "'".$listeSections."'";
	...
	*/
     ...
     * @return array
     */
    function AnniversairesDansxJours ($jours) {
        $connexion = Application::connectPDO(SERVEUR, BASE, NOM, MDP);
     ...
     public function randomPasswdEleve ($length){
        ...
        for ($n=0;$n<$length;$n++) {
			if ($n%2 == 0)
				...
				else $lettreSuivante = $this->randomLettre($voyelles);
     ...
     * @return array
     */
    function listeCoursGrpProf($listeNiveaux) {
		if (is_array($listeNiveaux))
			$listeNiveauxString = implode(",",$listeNiveaux);
            else $listeNiveauxString = $listeNiveaux;
        $connexion = Application::connectPDO(SERVEUR, BASE, NOM, MDP);
		...
		if ($resultat) {
			...
			while ($ligne = $resultat->fetch()) {
				...
				if (isset($listeCoursGrp[$coursGrp])) {
					...
					else $listeCoursGrp[$coursGrp] = $ligne;
				}
			}
		Application::DeconnexionPDO($connexion);
		return $listeCoursGrp;
	 ...
	 * @return array
	 */
	function listeCoursSuivis ($classe, $type='coursGrp') {
		$connexion = Application::connectPDO(SERVEUR, BASE, NOM, MDP);
     ...
     * @return array
     */
    function listeRemplacants ($listeCoursGrpProfs) {
     ...
     * @return array
     */
    function listeProfsCoursGrp($coursGrp) {
        $connexion = Application::connectPDO(SERVEUR, BASE, NOM, MDP);
    ...
    * @param $coursGrp
    */
    function ajouterProfsCoursGrp($listeProfs, $coursGrp) {
        $nbResultats = 0;
        if ($listeProfs) {
            $connexion = Application::connectPDO(SERVEUR, BASE, NOM, MDP);
	 ...
	 *
	 * @return integer : le nombre d'enregistrements effectués dans la BD
	 */
	 ...
	 * @return integer : le nombre d'élèves effectivement supprimés.
	 */
	 function supprEleves($listeEleves) {
		$connexion = Application::connectPDO(SERVEUR, BASE, NOM, MDP);
