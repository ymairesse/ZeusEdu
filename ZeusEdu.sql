...
('edt', 'EDT emploi du temps', 'edt/index.php', 'edt.png', 1, 98);
...
ALTER TABLE `didac_config`
  ...
  INSERT INTO `didac_config` (`ordre`, `parametre`, `label`, `size`, `valeur`, `signification`, `domaine`) VALUES
  (18, 'NOMNOREPLY', 'Nom adresse No Reply', 30, 'Merci de ne pas \'répondre\'', 'Nom de l\'adresse pour la diffusion de mails \"no reply\"', 'admin'),
  (18, 'NOREPLY', 'Adresse No Reply', 40, 'ne_pas_repondre@isnd.be', 'Adresse pour la diffusion de mails \"no reply\"', 'admin'),
  (2, 'VILLE', 'Ville', 45, '1070 - Bruxelles', 'Code postal et Localité de l\'école', 'admin'),
  (16, 'TELEPHONE', 'Téléphone', 20, '02/521.04.41', 'Téléphone général de l\'école', 'admin'),
  (15, 'SITEWEB', 'Adresse web', 45, 'http://www.isnd.be', 'Adresse URL du site web de l\'école', 'admin'),
  (14, 'SECTIONS', 'Sections', 40, 'GT,TT,TQ,G', 'Liste des sections proposées dans l\'école', 'bulletin'),
  (13, 'PERIODESDELIBES', 'Périodes de délibés', 20, '2,4', 'Numéro des périodes de délibérations', 'bulletin'),
  (12, 'PERIODEENCOURS', 'Période en cours', 2, '4', 'Numéro de la période en cours', 'bulletin'),
  (11, 'NOMSPERIODES', 'Noms des périodes', 60, 'Toussaint,Noël,Pâques,Juin', 'Noms des différentes périodes de cours', 'bulletin'),
  (10, 'NBPERIODES', 'Nombre de périodes', 2, '4', 'Nombre de périodes de l\'année scolaire (bulletin)', 'bulletin'),
  (9, 'MAXIMAGESIZE', 'Images', 9, '200000', 'Taille maximale (en Ko) des image en upload (trombinoscopes)', 'bulletin'),
  (5, 'LISTENIVEAUX', 'Niveaux d\'étude', 12, '1,2,3,4,5,6', 'Liste des niveaux d\'études existants', 'bulletin'),
  (1, 'ECOLE', 'Nom de l\'école', 30, 'Institut des Soeurs de Notre-Dame', 'Nom de l\'école', 'bulletin'),
  (6, 'DIRECTION', 'Direction', 30, 'Arnaud Arys, Directeur', 'Nom et titre de la direction (rapport de compétences)', 'bulletin'),
  (8, 'COTENULLE', 'Mentions nulles', 40, 'nr,NR', 'Cote valant pour 0 au carnet de cotes', 'bulletin'),
  (3, 'COMMUNE', 'Commune', 45, 'Anderlecht', 'Commune de l\'école', 'bulletin'),
  (7, 'COTEABS', 'Mentions d\'absence', 40, 'ne,NE,abs,cm,ABS,CM,EXC,exc,remd', 'Cote valant pour absence justifiée au carnet de cotes', 'bulletin'),
  (17, 'ANNEESCOLAIRE', 'Année scolaire', 9, '2020-2021', 'Année scolaire en cours (deux millésimes séparés par un tiret)', 'bulletin'),
  (4, 'ADRESSE', 'Adresse', 45, '40, rue de Veeweyde', 'Adresse de l\'école', 'bulletin'),
  (20, 'DISCLAIMER', 'Disclaimer', 80, 'http://www.isnd.be/disclaimer/disclaimer.htm', 'Clause de non responsabilité pour les mails sortants', 'admin'),
  (21, 'ANNEEDEGRE', 'Années de fin de degré', 20, '2,4,6', 'Années d\'études avec évaluation par degré (séparées par des virgules)', 'admin'),
  (25, 'ADRESSETHOT', 'Adresse Thot', 60, 'http://isnd.be/thot', 'Adresse de la plate-forme Thot adossée', 'admin'),
  (9, 'MENTIONSTEXTE', 'Mentions textuelles', 40, 'TI,I,D,S,AB,B,TB,E', 'Mentions textuelles sans calcul pour bulletin et carnet de cotes', 'admin'),
  (14, 'DELIBESTQ', 'Périodes de délibé TQ', 20, '2,4', 'Périodes de délibération dans la section TQ', 'bullTQ'),
  (14, 'PERIODETQ', 'Périodes en cours section TQ', 2, '4', 'Période en cours dans la section TQ', 'bullTQ'),
  (13, 'NBPERIODESTQ', 'Nombre de périodes TQ', 2, '4', 'Nombre de périodes de l\'année scolaire en section TQ', 'bullTQ'),
  (14, 'LISTENIVEAUXTQ', 'Liste des niveaux TQ', 20, '5,6', 'Liste des niveaux d\'étude en section TQ', 'bullTQ'),
  (15, 'LISTESECTIONSBULL', 'Liste des sections pour le bulletin', 20, 'TT,,GT', 'Liste des sections impliquées dans le bulletin (non TQ)', 'bulletin');
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Informations pour la journée';

  ALTER TABLE `didac_EDTinfos`
  ADD PRIMARY KEY (`type`,`idEDTinfo`,`date`),
  ADD KEY `idEDTinfo` (`idEDTinfo`);

  ALTER TABLE `didac_EDTinfos`
    MODIFY `idEDTinfo` tinyint NOT NULL AUTO_INCREMENT COMMENT 'Identifiant de l''information';

CREATE TABLE `didac_EDTprofsABS` (
  `acronyme` varchar(7) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
  `startTime` datetime NOT NULL COMMENT 'Jour et Heure de début du cours',
  `endTime` datetime DEFAULT NULL COMMENT 'Jour et Heure de fin du cours',
  `date` varchar(10) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL COMMENT 'Date (from startTime)',
  `heure` varchar(5) CHARACTER SET utf8 COLLATE utf8_unicode_520_ci NOT NULL COMMENT 'Heure de cours (from startTime)',
  `statutAbs` enum('ABS','indisponible') CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT 'ABS' COMMENT 'Statut de l''absence',
  `local` varchar(60) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL COMMENT 'local',
  `matiere` varchar(60) CHARACTER SET utf8 COLLATE utf8_unicode_ci DEFAULT NULL COMMENT 'Matière enseignée',
  ...
  `parties` varchar(60) CHARACTER SET utf8 COLLATE utf8_unicode_ci DEFAULT NULL COMMENT 'Parties de classes',
  `eduProf` varchar(7) CHARACTER SET utf8 COLLATE utf8_unicode_ci DEFAULT NULL COMMENT 'acronyme du prof ou de l''éducateur en charge',
  `remarque` varchar(80) CHARACTER SET utf8 COLLATE utf8_unicode_ci DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

ALTER TABLE `didac_EDTprofsABS`
...
CREATE TABLE `didac_thotRpRv` (
    ...
    `statut` enum('prof','dir') CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT 'prof' COMMENT 'Statut du membre du personnel',
    `matricule` int DEFAULT NULL COMMENT 'matricule de l''élève',
