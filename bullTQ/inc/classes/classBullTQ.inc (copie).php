...
class bullTQ {
	...
	/**
	 * renvoie la liste des cotes globlaes pour un coursGrp donné et une période donnée
	...
	public function listeCoursGrpEleves($listeEleves) {
		...
		$sql .= "ORDER BY nbheures DESC, statut DESC, rang, libelle";
	 ...
	 * @return void() : crée un fichier PDF 
	 */
	...
	$pdf->MultiCell(20,5,"page $page ",0,'R',0);
	}
	 ...
	 * @return void()
	 */
	private function cotesGlobalesPDF ($pdf, $cotesGlobales) {
		$pdf->SetLineWidth(0.2);
		$pdf->SetFont('Arial','',9);
		$texte = $this->utf8('Mentions globales pour la période');
		$pdf->Cell(50, 5, $texte, 0, 1, 'L', false);
		$pdf->SetFont('Arial','',7);
		// mentions
		$pdf->Cell(24, 5, 'TJ >', 1, 0, 'C', true);
		$pdf->Cell(24, 5, $this->utf8($cotesGlobales['Tj']), 1, 0, 'C', false);
		
		$pdf->Cell(24, 5, 'Examen >', 1, 0, 'C', true);
		$pdf->Cell(24, 5, $cotesGlobales['Ex'], 1, 0, 'C', false);
		$pdf->Cell(24, 5, $this->utf8('Période >'), 1, 0, 'C', true);
		$pdf->Cell(24, 5, $cotesGlobales['periode'], 1, 0, 'C', false);
		$pdf->Cell(24, 5, 'Global >', 1, 0, 'C', true);
		$pdf->Cell(24, 5, $cotesGlobales['global'], 1, 0, 'C', false);
	...
	private function cotesGenerauxPDF ($pdf, $competences, $cotes) {
		...
		$pdf->Cell(132, 5, $texte, 1, 0, 'C', true);
		$pdf->Cell(30, 5, 'TJ', 1, 0, 'C', true);
		$pdf->Cell(30, 5, 'EX', 1, 1, 'C', true);
		...
		foreach ($cotes as $idComp=>$lesCotes) {
			if (($lesCotes['Tj'] != '') || ($lesCotes['Ex'] != '')) {
				...
				$pdf->Cell(132, 5, $texteCompetence, 1, 0, 'R', false);
				$pdf->Cell(30, 5, $texteTJ, 1, 0, 'C', false);
				$pdf->Cell(30, 5, $texteEx, 1, 1, 'C	', false);
				}
		}
	}
	 ...
	 * @return void()
	 */
	private function commentProfPDF ($pdf, $commentaire) {
		$pdf->SetLineWidth(0.2);
		$pdf->SetFont('Arial','',9);
	...
	private function createBulletinEleve ($pdf, $infoPerso, $bulletin) {
		...
		foreach ($listeCoursGrp as $coursGrp=>$dataCours) {
			...
			if (isset($listeCotesGlobales[$bulletin][$coursGrp][$matricule]))
				$this->cotesGlobalesPDF($pdf, $listeCotesGlobales[$bulletin][$coursGrp][$matricule]);
				else $this->cotesGlobalesPDF($pdf, array('Tj'=>'', 'Ex'=>'', 'periode'=>'', 'global'=>''));
			...
			$this->commentProfPDF($pdf, $commentaire);
