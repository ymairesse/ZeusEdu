...
class bullTQ {
	...
	*/
	 ...
	 * @return array
	*/
	function cotesEleve ($matricule, $listePeriodes) {
	...
	*/
	 ...
	 * @return array
	 */
	function mentionsManuelles ($matricule) {
		$connexion = Application::connectPDO(SERVEUR, BASE, NOM, MDP);
	 ...
	 * @return void() : crée le fichier PDF
	 */
	 ...
	 * @return void()
	 */
	function piedPageFinal ($pdf, $page) {
		$pdf->SetFont('Arial','',8.5);
		$pdf->SetXY(6,270);
	 ...
	 * @return integer position Y max atteinte
	 */
	...
	private function createBulletinEleve ($pdf, $infoPerso, $bulletin) {
		...
		foreach ($listeCoursGrp as $coursGrp=>$dataCours) {
			...
			if (isset($listeCotesGlobales[$bulletin][$coursGrp][$matricule]))
				...
				else $yGlobal = $this->cotesGlobalesPDF($pdf, $yBase, array('Tj'=>'', 'Ex'=>'', 'periode'=>'', 'global'=>''));
