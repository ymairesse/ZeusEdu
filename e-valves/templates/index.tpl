...
$(document).ready(function(){
	...
	if ($("#corpsPage form").length != 0)
		$("#corpsPage form input:visible:enabled, #corpsPage form select:visible:enabled").not('.datepicker,.timepicker').first().focus();
		else
		$("form input:visible:enabled, #corpsPage form select:visible:enabled").not('.datepicker,.timepicker').first().focus();	
