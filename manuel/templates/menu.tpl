<ul id="menuBas">
	<li class="titreMenu">Manuel d'utilisation</li>
    <li><a href="../index.php"><img src="../images/homeIco.png" title="Retour"></a></li>
    <li><a href="index.php"><img src="images/manuelIco.png" title="Accueil" alt="Man"></a></li>
	<li><a href="javascript:void(0)">Infos générales</a>
		<ul>
			<li><a href="index.php?page=info">Infos</a></li>
			<li><a href="index.php?page=info&amp;cible=ip">Adresse IP</a></li>
			<li><a href="index.php?page=info&amp;cible=licence">Licence du logiciel</a></li>
		</ul>
	</li>

</ul>
