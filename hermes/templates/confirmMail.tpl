<div class="container">
	...
	<div id="cadre">
		...
		<p><strong class="enteteMail">Destinataire(s):</strong> {if $nbMails > 4}
			<span class="champ">{$nbMails} destinataires</span> {else}
			<span class="champ">{$destinatairesString}</span> {/if}
		</p>
		<strong>Texte</strong>
		<br>
		...
		<br> {assign var=n value=0} {foreach from=$detailsMail.files key=wtf item=data} {if $data.error != 4} {assign var=n value=n+1}
		<a href="upload/{$acronyme}/{$data.name}" target="_blank">{$data.name}</a>&nbsp;&nbsp; {/if} {/foreach} {if $n==0} aucun {/if}
	</div>
